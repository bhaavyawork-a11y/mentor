  ...
  return (
    <div style={{ padding: 32, maxWidth: 720 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0A3323", margin: "0 0 4px" }}>Services</h1>
          <p style={{ fontSize: 13, color: "#839958", margin: 0 }}>What you offer and at what price. Only active services appear to mentees.</p>
        </div>
      ...
      {services.length === 0 && !showForm && (
        <div style={{ marginBottom: 24 }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: "#0A3323", marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.5px" }}>
            Quick-add a preset
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 10 }}>
            {SERVICE_PRESETS.map(p => (
              <button key={p.title} onClick={() => openNew(p)} style={{
                backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 12,
              ...
              >
                ...
                <div style={{ fontSize: 12, fontWeight: 700, color: "#0A3323", marginBottom: 4 }}>{p.title}</div>
                <div style={{ fontSize: 11, color: "#839958" }}>{p.duration_mins} min · {formatINR(p.price_cents)}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Add/Edit form */}
      {showForm && (
        <div style={{ backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 16, padding: 24, marginBottom: 24 }}>
          <h2 style={{ fontSize: 15, fontWeight: 800, color: "#0A3323", margin: "0 0 20px" }}>
            {editing ? "Edit service" : "New service"}
          </h2>

          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#0A3323", marginBottom: 6 }}>Title</label>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#0A3323", marginBottom: 6 }}>Description (optional)</label>
            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
              placeholder="What's included? What will the mentee get out of this?"
              rows={3}
              style={{ width: "100%", boxSizing: "border-box", border: "1px solid #e8e4ce", borderRadius: 10, padding: "10px 14px", fontSize: 13, fontFamily: "inherit", outline: "none", resize: "vertical", color: "#1a1a1a" }} />
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 20 }}>
            <div>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#0A3323", marginBottom: 6 }}>Type</label>
              <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}
                style={{ width: "100%", border: "1px solid #e8e4ce", borderRadius: 10, padding: "10px 14px", fontSize: 13, fontFamily: "inherit", outline: "none", backgroundColor: "#fff", color: "#1a1a1a" }}>
                {Object.entries(TYPE_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </div>
            <div>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#0A3323", marginBottom: 6 }}>Duration (mins)</label>
              <input type="number" min={15} max={180} step={15}
                value={form.duration_mins} onChange={e => setForm(f => ({ ...f, duration_mins: e.target.value }))}
                style={{ width: "100%", boxSizing: "border-box", border: "1px solid #e8e4ce", borderRadius: 10, padding: "10px 14px", fontSize: 13, fontFamily: "inherit", outline: "none", color: "#1a1a1a" }} />
            </div>
            <div>
              <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#0A3323", marginBottom: 6 }}>Price (₹)</label>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
            ...
            <label htmlFor="active" style={{ fontSize: 13, color: "#1a1a1a" }}>Active — visible to mentees</label>
          </div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={() => setShowForm(false)} style={{
              backgroundColor: "transparent", border: "1px solid #e8e4ce", color: "#555",
      ...
      {services.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {services.map(s => (
            <div key={s.id} style={{
              backgroundColor: "#fff", border: `1px solid ${s.is_active ? "#e8e4ce" : "#f0ede0"}`,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  <p style={{ fontSize: 14, fontWeight: 700, color: "#1a1a1a", margin: 0 }}>{s.title}</p>
                  <span style={{
              ...
              <div style={{ fontSize: 16, fontWeight: 800, color: "#0A3323", flexShrink: 0 }}>
                {formatINR(s.price_cents)}
              </div>
              <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                <button onClick={() => openEdit(s)} style={{
                  fontSize: 12, backgroundColor: "#f5f5f0", color: "#555", border: "none",
