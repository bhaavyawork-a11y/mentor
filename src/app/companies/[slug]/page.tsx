...
function Stars({ rating, size = 13 }: { rating: number; size?: number }) {
  return <span style={{ fontSize: size, fontWeight: 700, color: "#FDE68A" }}>{"★".repeat(Math.round(rating))}{"☆".repeat(5 - Math.round(rating))}</span>;
}

const OUTCOME_STYLE: Record<string, { bg: string; color: string }> = {
  "Got offer":  { bg: "#00C9A722", color: "#1B3A35" },
  "Rejected":   { bg: "#ff6b6b22", color: "#cc0000" },
  "Withdrew":   { bg: "#f0f0f0",   color: "#888" },
  "No decision":{ bg: "#FDE68A22", color: "#8a7200" },
};
...
function ReviewModal({ slug, onClose, onSaved }: { slug: string; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, backgroundColor: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ backgroundColor: "#fff", borderRadius: 20, padding: 28, width: "100%", maxWidth: 520, display: "flex", flexDirection: "column", gap: 16, maxHeight: "90vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <h2 style={{ fontSize: 16, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Write a review</h2>
        ...
        {([["overall","Overall",form.overall],["culture","Culture",form.culture],["wlb","Work-life balance",form.wlb],["growth","Growth",form.growth],["comp","Compensation",form.comp]] as [keyof typeof form, string, number][]).map(([k, label, val]) => (
          <div key={k}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {[1,2,3,4,5].map((n) => (
                <button key={n} onClick={() => f(k, n)} style={{ width: 36, height: 36, borderRadius: 8, backgroundColor: val >= n ? "#FDE68A" : "#f0f0f0", border: "none", cursor: "pointer", fontSize: 16 }}>{"★"}</button>
              ))}
            </div>
          </div>
        ))}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <label htmlFor="rec" style={{ fontSize: 13, color: "#555", cursor: "pointer" }}>Would recommend to a friend</label>
        </div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, backgroundColor: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ backgroundColor: "#fff", borderRadius: 20, padding: 28, width: "100%", maxWidth: 520, display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <h2 style={{ fontSize: 16, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>Add your experience</h2>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "#888" }}>✕</button>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
...
export default function CompanyPage() {
  ...
  if (!company) return <div style={{ padding: 32 }}><p style={{ color: "#888" }}>Company not found.</p></div>;
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 0" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 28 }}>
        <div style={{ width: 60, height: 60, borderRadius: 14, backgroundColor: "#FDE68A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 800, color: "#1a1a1a", flexShrink: 0 }}>
          {company.name[0]}
        </div>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>{company.name}</h1>
            <span style={{ fontSize: 11, fontWeight: 700, backgroundColor: "#f0f0f0", color: "#888", borderRadius: 99, padding: "3px 10px" }}>{company.stage}</span>
          </div>
          <p style={{ fontSize: 13, color: "#888", margin: "4px 0 0" }}>{company.industry} · {company.size} employees · Founded {company.founded}</p>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
            <Stars rating={avgRating} />
            <span style={{ fontSize: 12, color: "#888" }}>{avgRating.toFixed(1)} · {company.recommend}% recommend</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 4, backgroundColor: "#f5f5f5", padding: 4, borderRadius: 12, marginBottom: 28, overflowX: "auto" }}>
        {TABS.map(({ id, label }) => (
          <button key={id} onClick={() => setTab(id)} style={{ fontSize: 12, fontWeight: tab === id ? 700 : 500, color: tab === id ? "#1a1a1a" : "#888", backgroundColor: tab === id ? "#fff" : "transparent", border: "none", borderRadius: 10, padding: "8px 16px", cursor: "pointer", boxShadow: tab === id ? "0 1px 4px rgba(0,0,0,0.08)" : "none", whiteSpace: "nowrap" }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {tab === "overview" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ backgroundColor: "#fff", border: "1px solid #eee", borderRadius: 14, padding: 20 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 10px" }}>About</p>
            <p style={{ fontSize: 14, color: "#333", lineHeight: 1.7, margin: 0 }}>{company.description}</p>
            {company.website && <a href={company.website} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: "#00C9A7", marginTop: 10, display: "inline-block" }}>Visit website →</a>}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            {[["Culture", company.culture],["Work-life", company.wlb],["Growth", company.growth],["Compensation", company.compensation]].map(([label, val]) => (
              <div key={label as string} style={{ backgroundColor: "#fff", border: "1px solid #eee", borderRadius: 12, padding: 16, textAlign: "center" }}>
                <p style={{ fontSize: 20, fontWeight: 800, color: "#1a1a1a", margin: "0 0 4px" }}>{(val as number).toFixed(1)}</p>
                <Stars rating={val as number} size={11} />
                <p style={{ fontSize: 11, color: "#888", margin: "4px 0 0" }}>{label}</p>
              </div>
            ))}
          </div>
          <div style={{ backgroundColor: "#1B3A35", borderRadius: 14, padding: "18px 20px" }}>
            <p style={{ fontSize: 22, fontWeight: 800, color: "#00C9A7", margin: "0 0 4px" }}>{company.recommend}%</p>
      ...
      {tab === "interviews" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {/* Static summary */}
          <div style={{ backgroundColor: "#fff", border: "1px solid #eee", borderRadius: 14, padding: 20 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 14px" }}>Typical process</p>
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              {[["Avg rounds", `${company.avgRounds} rounds`],["Duration", company.interviewDuration],["Difficulty", company.interviewDifficulty],["Offer rate", `${company.offerRate}%`]].map(([label, val]) => (
                <div key={label as string}>
                  <p style={{ fontSize: 13, fontWeight: 800, color: "#1a1a1a", margin: "0 0 2px" }}>{val}</p>
          ...
          {experiences.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <p style={{ fontSize: 14, color: "#888" }}>No experiences yet. Be the first to share.</p>
            </div>
          ) : (
            experiences.map((exp) => (
              <div key={exp.id} style={{ backgroundColor: "#fff", border: "1px solid #eee", borderRadius: 12, padding: 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 700, color: "#1a1a1a", margin: 0 }}>{exp.role}</p>
                    <p style={{ fontSize: 12, color: "#888", margin: "2px 0 0" }}>Anonymous · {new Date(exp.created_at).toLocaleDateString("en-IN", { month: "short", year: "numeric" })}</p>
                  </div>
                  {exp.outcome && (
                ...
                <div style={{ display: "flex", gap: 16, marginBottom: 10 }}>
                  {exp.rounds && <span style={{ fontSize: 12, color: "#555" }}>{exp.rounds} rounds</span>}
                  {exp.duration && <span style={{ fontSize: 12, color: "#555" }}>{exp.duration}</span>}
                  {exp.difficulty && <span style={{ fontSize: 12, color: "#555" }}>{exp.difficulty}</span>}
                </div>
                {exp.content && <p style={{ fontSize: 13, color: "#333", lineHeight: 1.6, margin: 0 }}>{exp.content}</p>}
              </div>
            ))
          )}
        </div>
      ...
      {tab === "salaries" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Link href="/salaries" style={{ fontSize: 13, color: "#00C9A7", textDecoration: "none" }}>← See all salary data</Link>
          {companySalaries.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <p style={{ fontSize: 14, color: "#888" }}>No salary data for {company.name} yet. <Link href="/salaries" style={{ color: "#00C9A7" }}>Contribute yours →</Link></p>
            </div>
          ) : (
            companySalaries.map((s, i) => (
              <div key={i} style={{ backgroundColor: "#fff", border: "1px solid #eee", borderRadius: 12, padding: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 700, color: "#1a1a1a", margin: "0 0 4px" }}>{s.role}</p>
                    <div style={{ display: "flex", gap: 8 }}>
                      <span style={{ fontSize: 11, backgroundColor: "#f0f0f0", color: "#888", borderRadius: 99, padding: "2px 8px" }}>{s.city}</span>
                      <span style={{ fontSize: 11, backgroundColor: "#f0f0f0", color: "#888", borderRadius: 99, padding: "2px 8px" }}>{s.yoe} yrs exp</span>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ fontSize: 16, fontWeight: 800, color: "#1a1a1a", margin: "0 0 2px" }}>{formatINR(s.totalCTC)} CTC</p>
                    <p style={{ fontSize: 12, color: "#888", margin: 0 }}>{formatINR(s.baseSalary)} base</p>
                  </div>
                </div>
              </div>
            ))
      ...
      {tab === "reviews" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {reviews.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <p style={{ fontSize: 14, color: "#888" }}>No reviews yet. Be the first.</p>
            </div>
          ) : (
            reviews.map((r) => (
              <div key={r.id} style={{ backgroundColor: "#fff", border: "1px solid #eee", borderRadius: 12, padding: 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <div>
                    <Stars rating={r.overall_rating} />
                    {(r.title || r.tenure) && <p style={{ fontSize: 12, color: "#888", margin: "4px 0 0" }}>{[r.title, r.tenure].filter(Boolean).join(" · ")}</p>}
                  </div>
                  {r.recommend && <span style={{ fontSize: 11, fontWeight: 700, backgroundColor: "#00C9A722", color: "#1B3A35", borderRadius: 99, padding: "3px 10px" }}>Recommends ✓</span>}
                </div>
                {r.pros && <p style={{ fontSize: 13, color: "#333", margin: "0 0 8px" }}><strong>Pros:</strong> {r.pros}</p>}
                {r.cons && <p style={{ fontSize: 13, color: "#555", margin: 0 }}><strong>Cons:</strong> {r.cons}</p>}
              </div>
            ))
          )}
        </div>
      ...
      {tab === "jobs" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {companyJobs.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <p style={{ fontSize: 14, color: "#888" }}>No open roles from {company.name} in our database yet. <Link href="/jobs" style={{ color: "#00C9A7" }}>Browse all jobs →</Link></p>
            </div>
          ) : (
            companyJobs.map((job) => (
              <div key={job.id} style={{ backgroundColor: "#fff", border: "1px solid #eee", borderRadius: 12, padding: 18 }}>
                <p style={{ fontSize: 15, fontWeight: 800, color: "#1a1a1a", margin: "0 0 6px" }}>{job.title}</p>
                <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  {[job.location, job.type, job.experience].map((t) => (
                    <span key={t} style={{ fontSize: 11, backgroundColor: "#FAF7F2", color: "#888", borderRadius: 99, padding: "3px 10px" }}>{t}</span>
                  ))}
                </div>
                <p style={{ fontSize: 13, color: "#666", lineHeight: 1.5, margin: "0 0 12px" }}>{job.description}</p>
                <Link href="/jobs" style={{ fontSize: 12, fontWeight: 700, backgroundColor: "#1B3A35", color: "#00C9A7", borderRadius: 8, padding: "8px 16px", textDecoration: "none" }}>Apply now →</Link>
              </div>
            ))
          )}
        </div>
