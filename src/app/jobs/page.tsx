...
function JobCard({ job, profile }: { job: Job; profile: Record<string, unknown> | null }) {
  ...
  const matchColor = match
    ? match.matchScore >= 80 ? "#839958" : match.matchScore >= 60 ? "#F7F4D5" : "#D3968C"
    : "#F7F4D5";

  return (
    <div style={{ backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
      {/* Top */}
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 11, color: "#839958", margin: 0 }}>{job.location}</p>
        </div>
        <span style={{ fontSize: 10, fontWeight: 700, backgroundColor: "#83995822", color: "#0A3323", borderRadius: 99, padding: "3px 10px", flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 800, backgroundColor: matchColor + "33", color: matchColor === "#F7F4D5" ? "#8a7200" : matchColor, borderRadius: 99, padding: "4px 12px", flexShrink: 0, whiteSpace: "nowrap" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {[job.location, job.type, `${formatSalary(job.salaryMin)} – ${formatSalary(job.salaryMax)}`, job.experience].map((tag) => (
          <span key={tag} style={{ fontSize: 11, fontWeight: 600, backgroundColor: "#F9F7EC", color: "#839958", borderRadius: 99, padding: "4px 12px" }}>{tag}</span>
        ))}
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {job.tags.map((t) => (
          <span key={t} style={{ fontSize: 11, backgroundColor: "#e8e4ce", color: "#555", borderRadius: 6, padding: "3px 8px" }}>{t}</span>
        ))}
      </div>

      {/* Match expansion */}
      {matchOpen && (
        <div style={{ backgroundColor: "#f8fffe", border: "1px solid #83995833", borderRadius: 12, padding: 14 }}>
          {matchLoading ? (
            <p style={{ fontSize: 13, color: "#839958", margin: 0 }}>Analysing your profile…</p>
          ) : match ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {match.reasons.length > 0 && (
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#839958", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 6px" }}>Why you match</p>
                  {match.reasons.map((r, i) => <p key={i} style={{ fontSize: 12, color: "#333", margin: "0 0 4px", display: "flex", gap: 6 }}><span style={{ color: "#839958" }}>✓</span>{r}</p>)}
                </div>
              )}
              {match.skillGaps.length > 0 && (
                <div>
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#839958", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 6px" }}>Gaps to bridge</p>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <a
          href={job.applyUrl}
          style={{ fontSize: 11, fontWeight: 700, backgroundColor: "#0A3323", color: "#839958", borderRadius: 8, padding: "7px 14px", textDecoration: "none", display: "inline-block" }}
        >
          Apply now →
        </a>
        <button
          onClick={loadMatch}
          style={{ fontSize: 11, fontWeight: 700, backgroundColor: "transparent", color: "#839958", border: "1px solid #e8e4ce", borderRadius: 8, padding: "7px 14px", cursor: "pointer" }}
        >
...
function PillRow({ options, value, onChange }: { options: string[]; value: string; onChange: (v: string) => void }) {
  return (
    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
      {options.map((opt) => (
        <button key={opt} onClick={() => onChange(opt)} style={{ fontSize: 12, fontWeight: value === opt ? 700 : 500, backgroundColor: value === opt ? "#0A3323" : "#e8e4ce", color: value === opt ? "#839958" : "#555", border: "none", borderRadius: 99, padding: "6px 14px", cursor: "pointer", transition: "all 0.15s" }}>
          {opt}
        </button>
      ))}
    </div>
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "32px 0" }}>
      ...
      <div style={{ backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 16, padding: "18px 20px", marginBottom: 28, display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#839958", textTransform: "uppercase", letterSpacing: "0.5px", margin: 0, whiteSpace: "nowrap" }}>Sort:</p>
          <PillRow options={SORTS} value={sort} onChange={setSort} />
        </div>
      </div>

      {/* Count */}
      <p style={{ fontSize: 13, color: "#839958", margin: "0 0 16px" }}>{filtered.length} job{filtered.length !== 1 ? "s" : ""}</p>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            ...
            <p style={{ fontSize: 14, color: "#839958" }}>No jobs match your filters.</p>
          </div>
        ) : (
          filtered.map((job) => (
