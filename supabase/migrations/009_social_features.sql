...
-- Saved posts, reposts, DMs, media support, follow counts
-- Safe to run — all use IF NOT EXISTS / ADD COLUMN IF NOT EXISTS

-- ─── Media URLs on community_posts ───────────────────────────────────────────
...
CREATE POLICY "Participants can read conversations" ON public.dm_conversations FOR SELECT
  USING (
    EXISTS (
...
CREATE POLICY "Users see own participations" ON public.dm_participants FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Participants can read others"  ON public.dm_participants FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.dm_participants p2 WHERE p2.conversation_id = conversation_id AND p2.user_id = auth.uid()));
CREATE POLICY "Users can join conversations"  ON public.dm_participants FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users update own participation" ON public.dm_participants FOR UPDATE USING (auth.uid() = user_id);
...
CREATE POLICY "Participants can read messages" ON public.dm_messages FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.dm_participants WHERE conversation_id = dm_messages.conversation_id AND user_id = auth.uid()));
CREATE POLICY "Participants can send messages" ON public.dm_messages FOR INSERT
  WITH CHECK (
    auth.uid() = sender_id AND
    EXISTS (SELECT 1 FROM public.dm_participants WHERE conversation_id = dm_messages.conversation_id AND user_id = auth.uid())
  );
...
-- ─── Trigger: update conversation updated_at + last_message on new DM ─────────
CREATE OR REPLACE FUNCTION update_conversation_on_message()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  UPDATE public.dm_conversations
  SET updated_at = now(), last_message = LEFT(NEW.content, 80), last_message_at = now()
...
-- ─── Supabase Storage bucket policy (run after creating bucket 'post-media') ──
-- Create the bucket via Supabase dashboard: Storage → New bucket → "post-media" → Public
-- Then run this:
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES ('post-media', 'post-media', true, 52428800, ARRAY['image/jpeg','image/png','image/gif','image/webp','video/mp4','video/quicktime','video/webm'])
