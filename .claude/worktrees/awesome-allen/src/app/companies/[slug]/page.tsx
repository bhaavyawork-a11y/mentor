...
function Stars({ rating, size = 13 }: { rating: number; size?: number }) {
  return <span style={{ fontSize: size, fontWeight: 700, color: "#F7F4D5" }}>{"★".repeat(Math.round(rating))}{"☆".repeat(5 - Math.round(rating))}</span>;
}

const OUTCOME_STYLE: Record<string, { bg: string; color: string }> = {
  "Got offer":  { bg: "#83995822", color: "#0A3323" },
  "Rejected":   { bg: "#ff6b6b22", color: "#cc0000" },
  "Withdrew":   { bg: "#e8e4ce",   color: "#839958" },
...
export default function CompanyPage() {
  ...
  if (!company) return <div style={{ padding: 32 }}><p style={{ color: "#839958" }}>Company not found.</p></div>;
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 0" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 28 }}>
        <div style={{ width: 60, height: 60, borderRadius: 14, backgroundColor: "#F7F4D5", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 800, color: "#1a1a1a", flexShrink: 0 }}>
          {company.name[0]}
        </div>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: "#1a1a1a", margin: 0 }}>{company.name}</h1>
            <span style={{ fontSize: 11, fontWeight: 700, backgroundColor: "#e8e4ce", color: "#839958", borderRadius: 99, padding: "3px 10px" }}>{company.stage}</span>
          </div>
          <p style={{ fontSize: 13, color: "#839958", margin: "4px 0 0" }}>{company.industry} · {company.size} employees · Founded {company.founded}</p>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
            <Stars rating={avgRating} />
            <span style={{ fontSize: 12, color: "#839958" }}>{avgRating.toFixed(1)} · {company.recommend}% recommend</span>
          </div>
        </div>
      </div>
      ...
      {tab === "overview" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 14, padding: 20 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: "#839958", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 10px" }}>About</p>
            <p style={{ fontSize: 14, color: "#333", lineHeight: 1.7, margin: 0 }}>{company.description}</p>
            {company.website && <a href={company.website} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: "#839958", marginTop: 10, display: "inline-block" }}>Visit website →</a>}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            {[["Culture", company.culture],["Work-life", company.wlb],["Growth", company.growth],["Compensation", company.compensation]].map(([label, val]) => (
              <div key={label as string} style={{ backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 12, padding: 16, textAlign: "center" }}>
                ...
                <p style={{ fontSize: 11, color: "#839958", margin: "4px 0 0" }}>{label}</p>
              </div>
            ))}
          </div>
          <div style={{ backgroundColor: "#0A3323", borderRadius: 14, padding: "18px 20px" }}>
            <p style={{ fontSize: 22, fontWeight: 800, color: "#839958", margin: "0 0 4px" }}>{company.recommend}%</p>
      ...
      {tab === "interviews" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {/* Static summary */}
          <div style={{ backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 14, padding: 20 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: "#839958", textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 14px" }}>Typical process</p>
          ...
          {experiences.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <p style={{ fontSize: 14, color: "#839958" }}>No experiences yet. Be the first to share.</p>
            </div>
          ) : (
            experiences.map((exp) => (
              <div key={exp.id} style={{ backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 12, padding: 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 700, color: "#1a1a1a", margin: 0 }}>{exp.role}</p>
                    <p style={{ fontSize: 12, color: "#839958", margin: "2px 0 0" }}>Anonymous · {new Date(exp.created_at).toLocaleDateString("en-IN", { month: "short", year: "numeric" })}</p>
                  </div>
                  {exp.outcome && (
      ...
      {tab === "salaries" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Link href="/salaries" style={{ fontSize: 13, color: "#839958", textDecoration: "none" }}>← See all salary data</Link>
          {companySalaries.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <p style={{ fontSize: 14, color: "#839958" }}>No salary data for {company.name} yet. <Link href="/salaries" style={{ color: "#839958" }}>Contribute yours →</Link></p>
            </div>
          ) : (
            companySalaries.map((s, i) => (
              <div key={i} style={{ backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 12, padding: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 700, color: "#1a1a1a", margin: "0 0 4px" }}>{s.role}</p>
                    <div style={{ display: "flex", gap: 8 }}>
                      <span style={{ fontSize: 11, backgroundColor: "#e8e4ce", color: "#839958", borderRadius: 99, padding: "2px 8px" }}>{s.city}</span>
                      <span style={{ fontSize: 11, backgroundColor: "#e8e4ce", color: "#839958", borderRadius: 99, padding: "2px 8px" }}>{s.yoe} yrs exp</span>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <p style={{ fontSize: 16, fontWeight: 800, color: "#1a1a1a", margin: "0 0 2px" }}>{formatINR(s.totalCTC)} CTC</p>
                    <p style={{ fontSize: 12, color: "#839958", margin: 0 }}>{formatINR(s.baseSalary)} base</p>
                  </div>
                </div>
              </div>
            ))
      ...
      {tab === "reviews" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {reviews.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <p style={{ fontSize: 14, color: "#839958" }}>No reviews yet. Be the first.</p>
            </div>
          ) : (
            reviews.map((r) => (
              <div key={r.id} style={{ backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 12, padding: 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <div>
                    <Stars rating={r.overall_rating} />
                    {(r.title || r.tenure) && <p style={{ fontSize: 12, color: "#839958", margin: "4px 0 0" }}>{[r.title, r.tenure].filter(Boolean).join(" · ")}</p>}
                  </div>
                  {r.recommend && <span style={{ fontSize: 11, fontWeight: 700, backgroundColor: "#83995822", color: "#0A3323", borderRadius: 99, padding: "3px 10px" }}>Recommends ✓</span>}
                </div>
      ...
      {tab === "jobs" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {companyJobs.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <p style={{ fontSize: 14, color: "#839958" }}>No open roles from {company.name} in our database yet. <Link href="/jobs" style={{ color: "#839958" }}>Browse all jobs →</Link></p>
            </div>
          ) : (
            companyJobs.map((job) => (
              <div key={job.id} style={{ backgroundColor: "#fff", border: "1px solid #e8e4ce", borderRadius: 12, padding: 18 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  {[job.location, job.type, job.experience].map((t) => (
                    <span key={t} style={{ fontSize: 11, backgroundColor: "#F9F7EC", color: "#839958", borderRadius: 99, padding: "3px 10px" }}>{t}</span>
                  ))}
                </div>
                <p style={{ fontSize: 13, color: "#666", lineHeight: 1.5, margin: "0 0 12px" }}>{job.description}</p>
                <Link href="/jobs" style={{ fontSize: 12, fontWeight: 700, backgroundColor: "#0A3323", color: "#839958", borderRadius: 8, padding: "8px 16px", textDecoration: "none" }}>Apply now →</Link>
              </div>
            ))
          )}
        </div>
